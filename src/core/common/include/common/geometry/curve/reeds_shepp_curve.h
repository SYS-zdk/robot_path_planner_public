 ...
 * ********************************************************
 */
#ifndef RMP_COMMON_GEOMETRY_REEDS_SHEPP_CURVE_H_
#define RMP_COMMON_GEOMETRY_REEDS_SHEPP_CURVE_H_
...
public:
   ...
   */
  ReedsSheppCurve();
  ReedsSheppCurve(double step, double max_curv);
   ...
   */
  Point3d interpolate(int mode, double length, const Point3d& init_pose);
  ...
  /**
   * @brief Get the shortest Reeds-Shepp path length without sampling/interpolation.
   * @param start Initial pose (x, y, yaw)
   * @param goal  Target pose (x, y, yaw)
   * @param length_out Shortest path length in meters.
   * @return true if a valid path exists.
   */
  bool shortestPathLength(const Point3d& start, const Point3d& goal, double& length_out);
...
protected:
   ...
   */
  void _update(const std::vector<RSPath>& cur_paths, RSPath& best_path);
