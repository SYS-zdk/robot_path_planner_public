 ...
 * ********************************************************
 */
#ifndef RPP_COMMON_UTIL_VISUALIZER_H_
#define RPP_COMMON_UTIL_VISUALIZER_H_
...
public:
  ...
  /**
   * @brief Publish variable-radius disks (CYLINDER) plus center points (POINTS).
   * @param centers_with_radius_m Points in world frame; theta() is interpreted as radius in meters.
   */
  static void publishVariableRadiusDisks2d(const Points3d& centers_with_radius_m, const ros::Publisher& publisher,
                                           const std::string& frame_id, const std::string& ns,
                                           std_msgs::ColorRGBA centers_color = RED,
                                           std_msgs::ColorRGBA disks_color = DARK_GREEN,
                                           double centers_scale = 0.06, double disks_height = 0.01,
                                           int max_disks = 200, bool delete_all = true,
                                           bool disks_color_gradient = false,
                                           double radius_min_m = 0.0, double radius_max_m = 0.0);
...
public:
  ...
  static std_msgs::ColorRGBA BLUE;//自己加的
  enum MARKER_TYPE
  {
    CUBE = 0,
    SPHERE = 1,
...
using VisualizerPtr = rpp::common::structure::Singleton<Visualizer>;
