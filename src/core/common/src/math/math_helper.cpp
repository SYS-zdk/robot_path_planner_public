...
namespace math
{
double crossProd(const rpp::common::geometry::Vec2d& start_point, const rpp::common::geometry::Vec2d& end_point_1,
                 const rpp::common::geometry::Vec2d& end_point_2)
{
  return (end_point_1 - start_point).crossProd(end_point_2 - start_point);
}

double innerProd(const rpp::common::geometry::Vec2d& start_point, const rpp::common::geometry::Vec2d& end_point_1,
                 const rpp::common::geometry::Vec2d& end_point_2)
{
 ...
 */
std::vector<rpp::common::geometry::Vec2d> circleSegmentIntersection(const rpp::common::geometry::Vec2d& p1,
                                                                    const rpp::common::geometry::Vec2d& p2, double r)
{
  std::vector<rpp::common::geometry::Vec2d> i_points;
 ...
 */
double arcCenter(const rpp::common::geometry::Vec2d& pt_prev, const rpp::common::geometry::Vec2d& pt,
                 const rpp::common::geometry::Vec2d& pt_next, bool is_cusp, rpp::common::geometry::Vec2d* center)
{
  auto d1 = pt - pt_prev;
  ...
  rpp::common::geometry::Vec2d n1(-d1.y(), d1.x());
  rpp::common::geometry::Vec2d n2(-d2.y(), d2.x());
  double det1 = (mid1.x() + n1.x()) * mid1.y() - (mid1.y() + n1.y()) * mid1.x();
  double det2 = (mid2.x() + n2.x()) * mid2.y() - (mid2.y() + n2.y()) * mid2.x();
  rpp::common::geometry::Vec2d arc_center((det1 * n2.x() - det2 * n1.x()) / det, (det1 * n2.y() - det2 * n1.y()) / det);
  if (center != nullptr)
  {
 ...
 */
rpp::common::geometry::Vec2d tangentDir(const rpp::common::geometry::Vec2d& pt_prev,
                                        const rpp::common::geometry::Vec2d& pt,
                                        const rpp::common::geometry::Vec2d& pt_next, bool is_cusp)
{
  rpp::common::geometry::Vec2d center;
  ...
  if (std::isinf(center.x()) && std::isinf(center.y()))
  {
    ...
    auto result = rpp::common::geometry::Vec2d(pt_next_cusp.x() - pt_prev.x(), pt_next_cusp.y() - pt_prev.y());
 ...
 */
void sortPoints(std::vector<rpp::common::geometry::Vec2d>& points)
{
  ...
  rpp::common::geometry::Vec2d middle_point(sum_x / static_cast<double>(points.size()),
                                            sum_y / static_cast<double>(points.size()));
  std::sort(points.begin(), points.end(),
            [&](const rpp::common::geometry::Vec2d& pt1, const rpp::common::geometry::Vec2d& pt2) {
