 ...
 */
double ReedsSheppCurve::RSPath::len() const
{
  double path_length = 0.0;
  for (const auto& l : lengths_)
  {
    path_length += std::fabs(l);
  }
...
{
  return rpp::common::math::pi2pi(theta);
}
 ...
 */
Point3d ReedsSheppCurve::interpolate(int mode, double length, const Point3d& init_pose)
{
 ...
 */
bool ReedsSheppCurve::generation(const Point3d& start, const Point3d& goal, Points3d& path)
{
  path.clear();
  ...
  for (size_t j = 0; j < best_path.size(); j++)
  {
    ...
    {
      i += 1;
      const auto& pt = interpolate(m, l, { x, y, yaw });
      path_x[i] = pt.x(), path_y[i] = pt.y(), path_yaw[i] = pt.theta();
      l += d_l;
    }
    i += 1;
    const auto& pt = interpolate(m, seg_length, { x, y, yaw });
    path_x[i] = pt.x(), path_y[i] = pt.y(), path_yaw[i] = pt.theta();
  }

  // Remove unused tail capacity safely. Do NOT pop based on numeric values,
  // because a valid endpoint can legitimately have x==0 in the normalized frame.
  const int used = i + 1;
  if (used > 0 && used <= static_cast<int>(path_x.size()))
  {
    path_x.resize(static_cast<size_t>(used));
    path_y.resize(static_cast<size_t>(used));
    path_yaw.resize(static_cast<size_t>(used));
  }

  // coordinate transformation
  for (size_t i = 0; i < path_x.size(); i++)
  {
    path.emplace_back(cos(-syaw) * path_x[i] + sin(-syaw) * path_y[i] + sx,
                      -sin(-syaw) * path_x[i] + cos(-syaw) * path_y[i] + sy,
                      rpp::common::math::pi2pi(path_yaw[i] + syaw));
  }

  return true;
}

bool ReedsSheppCurve::shortestPathLength(const Point3d& start, const Point3d& goal, double& length_out)
{
  length_out = std::numeric_limits<double>::infinity();
  if (!(max_curv_ > 0.0))
  {
    return false;
  }
  ...
  const double syaw = start.theta();
  const double gx = goal.x();
  const double gy = goal.y();
  const double gyaw = goal.theta();

  // coordinate transformation (same as generation)
  const double dx = gx - sx;
  const double dy = gy - sy;
  const double dyaw = gyaw - syaw;
  const double x = (std::cos(syaw) * dx + std::sin(syaw) * dy) * max_curv_;
  const double y = (-std::sin(syaw) * dx + std::cos(syaw) * dy) * max_curv_;
  ...
  const double best = best_path.len();
  if (!(best < std::numeric_limits<double>::max()))
  {
    return false;
  }

  length_out = best / max_curv_;
  return std::isfinite(length_out);
}
 ...
 * @return true if generate successfully, else failed
 */
bool ReedsSheppCurve::run(const Points2d& points, Points3d& path)
{
  if (points.size() < 4)
  {
    return false;
 ...
 * @return true if generate successfully, else failed
 */
bool ReedsSheppCurve::run(const Points3d& points, Points3d& path)
{
  if (points.size() < 4)
  {
    return false;
 ...
 */
void ReedsSheppCurve::_update(const std::vector<RSPath>& cur_paths, RSPath& best_path)
{
