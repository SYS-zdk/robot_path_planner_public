...
bool CollisionChecker::inCollision(const unsigned int& i, const bool& traverse_unknown)
{
  ...
  return center_cost >= costmap_2d::INSCRIBED_INFLATED_OBSTACLE * factor_;
}
...
bool CollisionChecker::isInsideMap(const unsigned int& i)
{
  ...
  // NOTE: index is unsigned; 0 is a valid cell. Previous check (i > 0) incorrectly treated cell 0 as out-of-map.
  return (i < size);
}
