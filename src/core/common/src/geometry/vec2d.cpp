...
void Vec2d::normalize()
{
  ...
  if (l > rpp::common::math::kMathEpsilon)
  {
    x_ /= l;
    y_ /= l;
  }
...
Vec2d Vec2d::operator/(const double ratio) const
{
  CHECK_GT(std::abs(ratio), rpp::common::math::kMathEpsilon);
  return Vec2d(x_ / ratio, y_ / ratio);
}
...
{
    return (std::abs(x_ - other.x()) < rpp::common::math::kMathEpsilon &&
      std::abs(y_ - other.y()) < rpp::common::math::kMathEpsilon);
}
