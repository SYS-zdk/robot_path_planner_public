 ...
 * ********************************************************
 */
#ifndef RPP_PATH_PLANNER_PATH_PLANNER_H_
#define RPP_PATH_PLANNER_PATH_PLANNER_H_
...
public:
   ...
   */
  PathPlanner(costmap_2d::Costmap2DROS* costmap_ros, double obstacle_factor = 0.5); //new
  ...
  /**
   * @brief Optional hook for planner-specific debug information.
   *
   * Pipeline calls this after planning+toolchain to let concrete planners attach extra debug fields
   * without requiring the pipeline to include concrete planner headers.
   */
  virtual void appendDebugData(DebugData& /*data*/) const {}
  ...
  /**
   * @brief Sync cached costmap properties (size/pointer) from costmap_ros_.
   *
   * Some costmap setups (e.g. static map layers) may resize the underlying costmap after
   * planner construction (when the map message arrives). Planners that cache nx_/ny_/map_size_
   * must refresh them before using world2Map/grid2Index loops.
   */
  void syncCostmapInfo();
...
protected:
  ...
  std::shared_ptr<rpp::common::geometry::CollisionChecker> collision_checker_;  // gridmap collision checker
