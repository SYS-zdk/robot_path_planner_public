...
{
  int motion_model{ 3 };                   // Motion model. 2 for DUBIN and 3 for REEDS_SHEPP
  double goal_tolerance{ 0.125 };          // tolerance distance to goal [cells]
  ...
  double heuristic_weight{ 2.5 };          // Weight to apply to heuristic in f = g + w*h
  ...
  double minimum_turning_radius{ 0.4 };    // Minimum turning radius [cells]
  ...
  // Output path post-processing (in map cell units).
  // If > 0, consecutive points closer than this threshold will be dropped.
  double path_min_separation{ 0.0 };

  // Optional Voronoi-based potential (requires costmap VoronoiLayer plugin).
  // Set weight <= 0.0 to disable.
  double voronoi_heuristic_weight{ 0.0 };  // additive term to heuristic
  double voronoi_cost_weight{ 0.0 };       // multiplier in g-cost: dist * (1 + w * rho_v)
  double voronoi_alpha{ 1.0 };             // [m] potential falloff rate
  double voronoi_d0_max{ 2.0 };            // [m] beyond this obstacle distance, potential is 0
};
...
struct HybridMotionTable
{
  ...
  // optional Voronoi heuristic potential parameters
  double voronoi_heuristic_weight{ 0.0 };
  double voronoi_cost_weight{ 0.0 };  // ✅ FIXED: Changed from 0.5 to 0.0 (disabled by default)
  double voronoi_alpha{ 1.0 };
  double voronoi_d0_max{ 2.0 };

  // motion primitives
  std::shared_ptr<rpp::common::geometry::Curve> curve_gen;
...
public:
  ...
  typedef rpp::common::geometry::Point3d Pose;
  typedef rpp::common::geometry::Points3d Poses;
  ...
  bool isValid(const bool& traverse_unknown,
               const std::shared_ptr<rpp::common::geometry::CollisionChecker>& collision_checker);
  ...
  /**
   * @brief (Optional) Precompute Voronoi ridge/obstacle distance caches for heuristic potential.
   */
  static void resetVoronoiHeuristic(costmap_2d::Costmap2DROS* costmap_ros);
...
public:
  ...
  // Optional Voronoi heuristic caches (meters)
  static std::vector<float> voronoi_obstacle_dist_m;
  static std::vector<float> voronoi_ridge_dist_m;
