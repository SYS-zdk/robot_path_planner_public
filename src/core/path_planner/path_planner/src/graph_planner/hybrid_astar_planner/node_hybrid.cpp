...
std::vector<float> NodeHybrid::voronoi_obstacle_dist_m;
std::vector<float> NodeHybrid::voronoi_ridge_dist_m;
...
{
  // search_info.minimum_turning_radius is already in map cell units.
  const double min_turning_radius_cells = search_info.minimum_turning_radius;

  // if nothing changed, no need to re-compute primitives
  if (num_angle_quantization == search_info.dim_3_size &&
      std::fabs(min_turning_radius - min_turning_radius_cells) < 1e-9 &&
  ...
  voronoi_heuristic_weight = search_info.voronoi_heuristic_weight;
  voronoi_cost_weight = search_info.voronoi_cost_weight;
  voronoi_alpha = search_info.voronoi_alpha;
  voronoi_d0_max = search_info.voronoi_d0_max;

  // update
  num_angle_quantization = search_info.dim_3_size;
  min_turning_radius = min_turning_radius_cells;
  motion_model = MotionModel::DUBIN;

  if (num_angle_quantization < 1u || !(min_turning_radius > 0.0))
  {
    R_ERROR << "Invalid HybridMotionTable params: dim_3_size=" << num_angle_quantization
            << ", min_turning_radius_cells=" << min_turning_radius;
    projections.clear();
    return;
  }
  ...
  const double ratio_raw = std::sqrt(2.0) / (2.0 * min_turning_radius);
  const double ratio = std::max(-1.0, std::min(1.0, ratio_raw));
...
MotionPoses HybridMotionTable::getProjections(const NodeHybrid* node)
{
  ...
  const unsigned int heading_bin = static_cast<unsigned int>(node->pose().theta());
  for (unsigned int i = 0; i != projections.size(); i++)
  {
    const MotionPose& proj_motion_model = projections[i];
    double new_heading = static_cast<double>(heading_bin) + proj_motion_model.theta_;
    ...
    projection_list.emplace_back(delta_xs[i][heading_bin] + node->pose().x(),
                   delta_ys[i][heading_bin] + node->pose().y(), new_heading,
...
double NodeHybrid::getTraversalCost(const NodePtr& child)
{
  ...
  // Optional Voronoi potential term integrated into g-cost (edge cost).
  // Formula: cost' = cost * (1 + w * rho_v)
  // rho_v = (α/(α+d0)) * (dv/(d0+dv)) * ((d0-d0_max)/d0_max)^2  [when d0 <= d0_max]
  // This encourages paths to stay away from obstacles and prefer Voronoi skeleton
  if (std::isfinite(motion_table.voronoi_cost_weight) && motion_table.voronoi_cost_weight > 0.0 &&
      !voronoi_obstacle_dist_m.empty() && !voronoi_ridge_dist_m.empty())
  {
    const int x = static_cast<int>(std::round(child->pose().x()));
    const int y = static_cast<int>(std::round(child->pose().y()));
    if (x >= 0 && y >= 0)
    {
      const size_t idx = static_cast<size_t>(y) * static_cast<size_t>(motion_table.map_width) + static_cast<size_t>(x);
      if (idx < voronoi_obstacle_dist_m.size() && idx < voronoi_ridge_dist_m.size())
      {
        const double d0 = static_cast<double>(voronoi_obstacle_dist_m[idx]);
        const double dv = static_cast<double>(voronoi_ridge_dist_m[idx]);
        const double d0_max = motion_table.voronoi_d0_max;
        const double alpha = motion_table.voronoi_alpha;
        ...
        if (std::isfinite(d0) && std::isfinite(dv) && std::isfinite(d0_max) && d0_max > 0.0 &&
            std::isfinite(alpha) && alpha >= 0.0 && d0 <= d0_max)
        {
...
double NodeHybrid::getHeuristicCost(const Pose& node_coords, const Pose& goal_coords)
{
  ...
  double heuristic = std::max(obstacle_heuristic, dist_heuristic);

  // Optional Voronoi potential term (requires costmap VoronoiLayer plugin).
  if (std::isfinite(motion_table.voronoi_heuristic_weight) && motion_table.voronoi_heuristic_weight > 0.0 &&
      !voronoi_obstacle_dist_m.empty() && !voronoi_ridge_dist_m.empty())
  {
    const int x = static_cast<int>(std::round(node_coords.x()));
    const int y = static_cast<int>(std::round(node_coords.y()));
    if (x >= 0 && y >= 0)
    {
      const size_t idx = static_cast<size_t>(y) * static_cast<size_t>(motion_table.map_width) +
      ...
      if (idx < voronoi_obstacle_dist_m.size() && idx < voronoi_ridge_dist_m.size())
      {
        ...
        if (std::isfinite(d0) && std::isfinite(dv) && std::isfinite(d0_max) && d0_max > 0.0 && std::isfinite(alpha) &&
            alpha >= 0.0 && d0 <= d0_max)
        {
          // Potential in [0,1]:
          // - 0 on Voronoi ridge (dv=0)
          // - 1 near/in obstacles (d0~0)
          const double term1 = alpha / (alpha + std::max(0.0, d0));
          const double denom = std::max(1e-6, d0 + dv);
          const double term2 = std::max(0.0, dv) / denom;
          const double term3_raw = (d0 - d0_max) / d0_max;
          ...
          heuristic += motion_table.voronoi_heuristic_weight * rho_v;
        }
      }
    }
  }
                                        ...
                                        const double& obstacle_heuristic)
{
  ...
  // Fallback: if outside the lookup window, use a cheap lower-bound heuristic instead of 0.
  // This keeps the heuristic informative even far from the goal.
  double motion_heuristic = std::hypot(dx, dy);
  ...
  else if (obstacle_heuristic <= 0.0)
  {
    ...
    const double from_yaw = motion_table.getAngleFromBin(static_cast<unsigned int>(node_coords.theta()));
    const double to_yaw = motion_table.getAngleFromBin(static_cast<unsigned int>(goal_coords.theta()));
    Point3d from(node_coords.x(), node_coords.y(), from_yaw);
    Point3d to(goal_coords.x(), goal_coords.y(), to_yaw);
                                        ...
                                        const unsigned int& goal_y)
{
  ...
  // Optional: cache Voronoi-derived distances for heuristic augmentation.
  resetVoronoiHeuristic(costmap_ros_i);
}

void NodeHybrid::resetVoronoiHeuristic(costmap_2d::Costmap2DROS* costmap_ros_i)
{
  voronoi_obstacle_dist_m.clear();
  voronoi_ridge_dist_m.clear();
  ...
  const bool enable_voronoi =
      (std::isfinite(motion_table.voronoi_heuristic_weight) && motion_table.voronoi_heuristic_weight > 0.0) ||
      (std::isfinite(motion_table.voronoi_cost_weight) && motion_table.voronoi_cost_weight > 0.0);
  if (!enable_voronoi)
  {
    return;
  }

  const auto* costmap = costmap_ros_i->getCostmap();
  if (!costmap)
  {
    return;
  }
  ...
  auto* layered = costmap_ros_i->getLayeredCostmap();
  if (!layered)
  {
    return;
  }
  ...
  for (auto layer = layered->getPlugins()->begin(); layer != layered->getPlugins()->end(); ++layer)
  {
    auto candidate = boost::dynamic_pointer_cast<costmap_2d::VoronoiLayer>(*layer);
    if (candidate)
    {
      voronoi_layer = candidate;
      break;
    }
  }
  ...
  const unsigned int size_x = vor.getSizeX();
  const unsigned int size_y = vor.getSizeY();
  ...
  voronoi_obstacle_dist_m.resize(size, std::numeric_limits<float>::infinity());
  voronoi_ridge_dist_m.resize(size, std::numeric_limits<float>::infinity());

  std::queue<uint32_t> q;

  // Seed queue with Voronoi ridge pixels; also cache obstacle distance.
  for (unsigned int y = 0; y < size_y; ++y)
  {
    for (unsigned int x = 0; x < size_x; ++x)
    {
      const size_t idx = static_cast<size_t>(y) * size_x + x;
      voronoi_obstacle_dist_m[idx] =
          static_cast<float>(vor.getDistance(static_cast<int>(x), static_cast<int>(y)) * resolution);

      if (vor.isVoronoi(static_cast<int>(x), static_cast<int>(y)))
      {
        voronoi_ridge_dist_m[idx] = 0.0f;
        q.push(static_cast<uint32_t>(idx));
      }
    }
  }
  ...
  if (q.empty())
  {
    // Voronoi layer may not be updated yet.
    voronoi_obstacle_dist_m.clear();
    voronoi_ridge_dist_m.clear();
    return;
  }
  ...
  // Multi-source BFS to compute distance-to-ridge (4-connected) in meters.
  while (!q.empty())
  {
    const uint32_t cur = q.front();
    q.pop();
    const unsigned int cx = cur % size_x;
    const unsigned int cy = cur / size_x;
    const float cd = voronoi_ridge_dist_m[cur];
    ...
    for (int k = 0; k < 4; ++k)
    {
      ...
      const uint32_t nidx = static_cast<uint32_t>(static_cast<unsigned int>(ny) * size_x +
                                                  static_cast<unsigned int>(nx));
      const float nd = cd + static_cast<float>(resolution);
      if (nd < voronoi_ridge_dist_m[nidx])
      {
        voronoi_ridge_dist_m[nidx] = nd;
        q.push(nidx);
      }
    }
  }
...
void NodeHybrid::precomputeDistanceHeuristic(const MotionModel& motion_model, const HybridSearchInfo& search_info)
{
  ...
  {
    const double min_turning_radius = motion_table.min_turning_radius > 0.0 ? motion_table.min_turning_radius :
                                                                               search_info.minimum_turning_radius;
    motion_table.curve_gen = std::make_shared<DubinsCurve>(search_info.curve_sample_ratio, 1.0 / min_turning_radius);
  }
  else if (motion_model == MotionModel::REEDS_SHEPP)
  {
    ...
    motion_table.curve_gen =
        std::make_shared<ReedsSheppCurve>(search_info.curve_sample_ratio, 1.0 / min_turning_radius);
  }
  else
  {
 ...
 */
void HybridMotionTable::initReedsShepp(unsigned int& size_x_in, unsigned int& /*size_y_in*/, HybridSearchInfo& search_info)
{
  // search_info.minimum_turning_radius is already in map cell units.
  const double min_turning_radius_cells = search_info.minimum_turning_radius;
  ...
  curve_gen = std::make_shared<ReedsSheppCurve>(search_info.curve_sample_ratio, 1.0 / min_turning_radius);

  projections.clear();
  projections.emplace_back(d_dist, 0.0, 0.0, TurnDirection::FORWARD);
  projections.emplace_back(d_x, d_y, increments, TurnDirection::LEFT);
  projections.emplace_back(d_x, -d_y, -increments, TurnDirection::RIGHT);

  projections.emplace_back(-d_dist, 0.0, 0.0, TurnDirection::REVERSE);
  // When reversing, heading change is opposite of forward for the same steering.
  projections.emplace_back(-d_x, d_y, -increments, TurnDirection::REV_LEFT);
  projections.emplace_back(-d_x, -d_y, increments, TurnDirection::REV_RIGHT);
  ...
  for (unsigned int i = 0; i != projections.size(); i++)
  {
    ...
    for (unsigned int j = 0; j != num_angle_quantization; j++)
    {
      const double cos_theta = std::cos(bin_size * j);
      const double sin_theta = std::sin(bin_size * j);
      if (i == 0)
      {
        trig_values[j] = { cos_theta, sin_theta };
      }
  ...
  for (unsigned int i = 0; i != projections.size(); i++)
  {
    ...
    if (turn_dir != TurnDirection::FORWARD && turn_dir != TurnDirection::REVERSE)
    {
      const double arc_angle = projections[i].theta_ * bin_size;
      const double turning_rad = 0.5 * d_dist / (std::sin(arc_angle * 0.5));
      travel_costs[i] = turning_rad * arc_angle;
    }
    else
