...
void AStarFramework<NodeT>::setCollisionChecker(const std::shared_ptr<CollisionChecker>& collision_checker)
{
  ...
  // Provide costmap ROS to node for resolution-aware conversions.
  NodeT::costmap_ros = collision_checker->getCostmapROS();

  // Guard against invalid configuration (too few heading bins will break motion primitive math).
  ...
  if (size_x_ != size_x || size_y_ != size_y)
  {
    ...
    // Precompute distance heuristic once motion model is initialized (needs a valid turning radius in cells).
    NodeT::precomputeDistanceHeuristic(motion_model_, search_info_);
  }
