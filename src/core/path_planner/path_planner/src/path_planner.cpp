...
void PathPlanner::syncCostmapInfo()
{
  if (!costmap_ros_)
  {
    return;
  ...
  const int new_nx = static_cast<int>(cm->getSizeInCellsX());
  const int new_ny = static_cast<int>(cm->getSizeInCellsY());
  if (new_nx <= 0 || new_ny <= 0)
  {
    return;
  }

  // Keep cached size in sync with the underlying costmap.
