  ...
  if (path_found)
  {
    ...
    if (_getPlanFromPath(origin_path, plan))
    {
      ...
      if (show_safety_corridor_)
      {
        ...
        auto safety_corridor = std::make_unique<rpp::common::safety_corridor::RemovedCorridor>(costmap_ros_, 0.6, ackermann_cfg); //原0.5
        // auto safety_corridor = std::make_unique<rpp::common::safety_corridor::RemovedCorridor>(costmap_ros_, 0.5);
...
void PathPlannerNode::configurePlannerToolchain()
{
  ...
  if (planner_name_ == "a_star" || planner_name_ == "dijkstra" || planner_name_ == "gbfs")
  {
    ...
    // 启用RDP路径剪枝
    pruner_ = std::make_shared<RDPPathSimplifier>(0.22);
