 ...
 */
void Controller::setSize(int nx, int ny)
{
  nx_ = nx;
  ny_ = ny;
  ns_ = nx * ny;
 ...
 */
void Controller::setOrigin(double origin_x, double origin_y)
{
  origin_x_ = origin_x;
  origin_y_ = origin_y;
}
 ...
 */
void Controller::setResolution(double resolution)
{
...
Eigen::Vector3d Controller::getEulerAngles(geometry_msgs::PoseStamped& ps)
{
  tf2::Quaternion q(ps.pose.orientation.x, ps.pose.orientation.y, ps.pose.orientation.z, ps.pose.orientation.w);
  tf2::Matrix3x3 m(q);
...
double Controller::getLookAheadDistance(double vt)
{
  ...
  return rpp::common::math::clamp(lookahead_dist, min_lookahead_dist_, max_lookahead_dist_);
}
                                   ...
                                   geometry_msgs::PointStamped& pt, double& theta, double& kappa)
{
  ...
  else
  {
    ...
    rpp::common::geometry::Vec2d prev_p(px - rx, py - ry);
    rpp::common::geometry::Vec2d goal_p(gx - rx, gy - ry);
    std::vector<rpp::common::geometry::Vec2d> i_points =
      rpp::common::math::circleSegmentIntersection(prev_p, goal_p, lookahead_dist);
