...
│  │  ├─ 输入：初始路径 π₀，ESDF 代价场
│  │  └─ 输出：平滑全局参考路径 π*（实时性优先）
│  └─ 对应代码：removed_optimizer.cpp（第 8B 章）
│
...
**全局路径优化（共轭梯度）：**
- 共轭梯度优化器实现：`src/core/trajectory_planner/src/trajectory_optimization/removed_optimizer/removed_optimizer.cpp`
- 接口定义：`src/core/trajectory_planner/include/trajectory_planner/trajectory_optimization/removed_optimizer/removed_optimizer.h`
- 参数配置：`src/sim_env/config/trajectory_optimization/removed_optimizer_params.yaml`
...
| 第 8B 章 | 共轭梯度法（全局路径优化） | `removed_optimizer.cpp` |
...
实现在 `removed_optimizer.cpp` 中以如下方式计算：
...
```cpp
Vec2d RemovedOptimizer::_calObstacleTerm(const Vec2d& xi,
...
```cpp
Vec2d RemovedOptimizer::_calSmoothTerm(const Vec2d& xi_c2, const Vec2d& xi_c1,
...
```cpp
Vec2d RemovedOptimizer::_calCurvatureTerm(const Vec2d& xi_c1, const Vec2d& xi,
    const Vec2d& xi_p1)
{
    Vec2d gradient;
...
```cpp
bool RemovedOptimizer::optimize(const Points3d& waypoints)
{
    // ... 获取距离层 ...
    int iter = 0;
...
对应 `src/sim_env/config/trajectory_optimization/removed_optimizer_params.yaml`：
...
调用 `RemovedCorridor::decompose(pts, params)` 生成凸多边形序列，输入参数包括轮基、转向角限制、轨迹宽度、安全裕度等，考虑机器人的实际占有空间，对每个参考点膨胀出自由区域后使用凸包算法得到凸多边形，相邻多边形的重叠保证了通过任意相邻多边形交集的轨迹都满足约束。
